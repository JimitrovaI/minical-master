...
class Extra_model extends CI_Model {
	...
	function get_filtered_extras($company_id, $data)
	{
		$this->db->from('extra as e, extra_rate as er');
		$this->db->where('e.extra_id = er.extra_id');
		...
		if(isset($data['extra_name']) && !empty($data['extra_name'])){
			$this->db->like('e.extra_name', $data['extra_name']);
		}
		...
		if(isset($data['charge_type_id']) && !empty($data['charge_type_id'])){
			$this->db->where('e.charge_type_id', $data['charge_type_id']);
		}

		if(isset($data['min_rate'])){
			$this->db->where('er.rate >=', $data['min_rate']);
		}
		
		if(isset($data['max_rate']) && !empty($data['max_rate'])){
			$this->db->where('er.rate <=', $data['max_rate']);
		}
