...
class Booking extends MY_Controller
{
    ...
    function get_check_in_bookings_in_JSON()
    {
        $filters = array('state' => INHOUSE);
        $bookings = $this->Booking_model->get_bookings($filters, null, null, true);
        $data = array();
        foreach ($bookings as $booking) {
            $data[$booking['room_id']] = $booking;
        }
    ...
    function get_bookings_in_JSON()
    {
        ...
        foreach ($bookings as $booking) {
            ...
            if (
                $this->selling_date >= date('Y-m-d', strtotime($booking['check_out_date'])) &&
    ...
    function create_booking_AJAX()
    {
        ...
        //check overbooking
        if (!$is_duplicate && !(isset($data['isGroupBooking']) && (bool)$data['isGroupBooking'])) {
        ...
        if (
            isset($data['customers']['paying_customer']) &&
            ...
            if (isset($paying_customer['customer_id'])) {
                ...
                $paying_customer_id = $this->Customer_model->create_customer(
                    array(
                        ...
                        'vehicle_number' => $paying_customer['vehicle_number']
                    )
                );
        ...
        foreach ($data['rooms'] as $room) {
            ...
            $booking_data['taxed_rate'] = $room['taxed_rate'];
    ...
    function update_booking_AJAX()
    {
        ...
        if ($new_state == 2) {
            ...
            $this->Booking_room_history_model->update_check_out_date($latest_block, $new_data['rooms'][0]['check_out_date']);
        }
    ...
    function update_channel_manager()
    {
        ...
        if (isset($block)) {
            ...
            $req = Requests::post(
                $this->config->item('cm_url') . '/sync/update_rates_and_availability/',
                array(
    ...
    function _validate_booking_data($new_data, $old_data = '', $booking_id = '', $company_detail = array())
    {
        ...
        if (isset($new_data['rooms'])) {
            ...
            if (
                ...
                if (
                    $new_data['rooms'][0]['check_in_date'] != '' &&
