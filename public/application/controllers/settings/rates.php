...
class Rates extends MY_Controller
{
    ...
    function create_custom_rate_AJAX()
    {
        ...
        if ($rate_plan_id == null) {   // create new custom rate plan
    ...
    function create_rate_plan_AJAX()
    {
        ...
        if ($room_type_ids) {
            ...
            $rate_plan_data = array(
                ...
                'rental_hours' => $this->security->xss_clean($this->input->post('rental_hours')),
                'rate_amount' => $this->security->xss_clean($this->input->post('rate_amount')),
            );
    ...
    function replicate_rate_plan_AJAX()
    {
        ...
        $new_rate_plan_data = array(
            ...
            'rental_hours' => $rate_plan['rental_hours'],
            'charge_type_id' => $rate_plan['charge_type_id'],
...
*/
    ...
    function create_extra()
    {
        ...
        $product_categories = $this->Product_category_model->get_product_categories($this->company_id);
    ...
    {
        $filtered_extras = $this->input->post('filtered_extras');
        if (isset($filtered_extras) and !empty($filtered_extras)) {
            $data = array(
                'extra_name' => $this->security->xss_clean($filtered_extras['extra_name']),
                'category_id' => $this->security->xss_clean($filtered_extras['category_id']),
                'charge_type_id' => $this->security->xss_clean($filtered_extras['extra_charge_type_id']),
                'min_rate' => $this->security->xss_clean($filtered_extras['min_rate']),
                'max_rate' => $this->security->xss_clean($filtered_extras['max_rate']),
            );

            $extras = $this->Extra_model->get_filtered_extras($this->company_id, $data);
            ...
            $this->load->view('hotel_settings/rate_plan_settings/filter_extras', $data);
        }
    }
    ...
    function update_extras()
    {
        ...
        foreach ($updated_extras as $updated_extra) {
            ...
            $data = array(
                ...
                'category_id' => $this->security->xss_clean($updated_extra['category_id']),
