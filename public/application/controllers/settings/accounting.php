...
class Accounting extends MY_Controller
{
	...
	{		
		...
		if ($this->form_validation->run() == TRUE)
		{
			...
			$result = $this->Product_category_model->create_payment_type($this->company_id, $product_category);
		}
		
		$data = array();
		
		//get payment types
		if(!is_null($product_categories = $this->Product_category_model->get_product_categories($this->company_id)))
		{
		...
		$data['js_files'] = array(
			base_url() . 'js/jquery.jeditable.mini.js',
			base_url() . auto_version('js/hotel-settings/product_categories.js')
		);
		$data['selected_sidebar_link'] = 'Product Category';
	
		
		$data['main_content'] = 'hotel_settings/accounting_settings/product_category_settings';
		$this->load->view('includes/bootstrapped_template', $data);
	...
	function create_product_category() {
		...
		$product_category_id = $this->Product_category_model->create_product_category($this->company_id, 'Click to edit');
		$this->_create_accounting_log("Create New Product Category ( [ID {$product_category_id}])");
		echo $product_category_id;
	}
	
	function change_product_category_name()
	{
		$product_category_id = $this->input->post('product_category_id');
		$product_category_name = $this->input->post('product_category_name');
		
		$data = array ('product_category' => $product_category_name);
		
		if ($this->Product_category_model->update_product_category($product_category_id, $data, $this->company_id))
		{
			$this->_create_accounting_log("Update Product Category Name( [ID {$product_category_id}])");
			echo $product_category_name; //TODO: verify update.
		}
		else
		{
			echo l('An occured changing the product category name. Please contact adminstrator if this persists.',true);
		}
	}
	...
	function delete_product_category()
	{
		...
		if ($this->Product_category_model->update_product_category($product_category_id, $data, $this->company_id))
		{
			$data = array ('isSuccess'=> TRUE, 'message' => 'Product category deleted');
			$this->_create_accounting_log("Delete Product Category( [ID {$product_category_id}])");
			echo json_encode($data);
		}
		else
		{
			$data = array ('isSuccess'=> FALSE, 'message' => 'Product category delete fail');
			echo json_encode($data);
		}		
	}
