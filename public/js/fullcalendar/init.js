...
$(function () {
    ...
    innGrid.getRooms(innGrid.buildRoomBoard);
...
//Create Room Board
innGrid.buildRoomBoard = function (rooms) {
    // $('#calendar').html('');

    $.getJSON(getBaseURL() + 'booking/get_check_in_bookings_in_JSON', function (data) {
        var bookings = data;
        var rooms_html = '';
        var l = rooms.length, i;
        for (i = 0; i < l; i++) {
            var room_name = rooms[i].name;
            var room_id = rooms[i].id
            var room_type = rooms[i].room_type;
            var room_type_id = rooms[i].room_type_id;
            var room_status = rooms[i].status;
            var available = true;

            var booking_id = '', booking_notes = '', customer_id = '', vehicle_number = '', customer_name = '', check_in_time = '', check_out_time = '', balance = '';
            var timer_container_class = '';

            var check_in_room_cls = '';

            if (bookings[room_id]) {
                // console.log(bookings[room_id]);
                available = false;
                check_in_room_cls = 'booking_room_container';
                timer_container_class = 'busy_room_bg';
                booking_id = bookings[room_id]['booking_id']
                booking_notes = bookings[room_id]['booking_notes']
                customer_id = bookings[room_id]['booking_customer_id']
                customer_name = bookings[room_id]['customer_name']
                vehicle_number = bookings[room_id]['vehicle_number']
                balance = bookings[room_id]['balance']
                check_in_time = bookings[room_id]['check_in_date']
                check_out_time = bookings[room_id]['check_out_date']

                var now = moment(new Date()); //todays date
                var start = moment(check_in_time, "YYYY-MM-DD HH:mm:ss");
                var end = moment(check_out_time, "YYYY-MM-DD HH:mm:ss"); // another date
                var tminutes = Math.floor(moment.duration(end.diff(start)).asMinutes());
                var rminutes = Math.floor(moment.duration(end.diff(now)).asMinutes());
                if (rminutes < ALERT_THRESHOLD) {
                    timer_container_class = 'danger_room_bg';
                } else if (rminutes < WARNING_THRESHOLD) {
                    timer_container_class = 'warning_room_bg';
                }
                // console.log(tminutes, cminutes)
            }
            room_html = '';
            room_html += '<div class="p-4 m-1 btn room-container-block ' + timer_container_class + ' '+check_in_room_cls+' " data-room-id="' + room_id + '" data-room-type-id="' + room_type_id + '" data-room_name="' + room_name + '" data-booking_id="' + booking_id + '" data-customer_name="' + customer_name + '" data-check_in="' + check_in_time + '" data-vehicle_number="' + vehicle_number + '" data-check_out="' + check_out_time + '" data-balance="' + balance + '" data-booking_notes="' + booking_notes + '" >';
            room_html += '<img class="room-status-icon" src="' + getBaseURL() + 'images/' + room_status + '.png">';
            room_html += '<div class="roomtimer" data_room_id="' + room_id + '" ';
            if (available == false) {
                room_html += ' data-timer_length = "' + tminutes + '" data-timer_left = "' + rminutes + '" ></div>'
            } else {
                room_html += '></div>';
            }

            room_html += '<div class="room_name_type" data-resource-id="' + room_id + '" >' + room_name + ' (' + room_type + ') <div class="edit-icon"></div></div>'
            room_html += '</div>';

            rooms_html += room_html;
        }



        $('#rooms_container').html(rooms_html);

        $(".room_name_type").click(function (event) {
            innGrid.populateRoomEditModal($(this))
            event.stopPropagation();
        });

        $(".room_name_type").mousedown(function (event) {
            console.log('mousedown')
            event.stopPropagation();
        });

        $(".room-container-block").click(function (event) {

            var room_id = $(this).data('room-id')
            var room_type_id = $(this).data('room-type-id')
            if($(this).hasClass('booking_room_container')){
                ...
                $(this).openBookingModal({
                    checkInDate: moment(new Date()).format("YYYY-MM-DD HH:mm:00"),
                    checkOutDate: moment(new Date()).format("YYYY-MM-DD HH:mm:00"),
                    roomID: room_id,
        ...
        if (innGrid.isDisplayTooltip) {
            $(".booking_room_container").mouseenter(function(e) {
                var checkInDate = $(this).data('check_in')
                var checkOutDate =$(this).data('check_out')
                ...
                var resDetails = '<div class="tooltip-reservation" style="display:none;background-color: white;position:absolute;z-index:10001;padding:8px ; font-size: 12px; border: 2px solid rgba(0,0,0,0.5)">' +
                    '<p style="margin-bottom:8px;text-transform: capitalize;"><strong>' + $(this).data('customer_name') + '</strong> ('+ vehicle_number +')</p>' +
                    '<p><strong>' + "Room" + ':</strong> ' + $(this).data('room_name') + '</p>' +
                    '<p><span><strong>' + "Check In" + ':</strong> ' + checkInDate + '</span> </p>' +
                    '<p><span><strong>' + "Check Out" + ':</strong> ' + checkOutDate + '</span></p>' +
                    '<p style="margin-bottom:8px;"> <span><strong>' + "Amount Due" + ':</strong> ' + parseFloat($(this).data('balance')) + '</span></p>' +
                    '</div>';
            ...
            $(".booking_room_container").mouseover(function (e) {
            ...
            $(".booking_room_container").mouseleave(function (e) {
                // $(this).css('z-index', 8);
        ...
        $('.roomtimer').each(function () {
            if ($(this).data('timer_length')) {
                var timeLimit = $(this).data('timer_length') * 1;
                var timeLeft = $(this).data('timer_left') * 1;
                if (timeLimit && timeLimit > 0) {
                    $(this).html(`<div class="base-timer">
                    ...
                    innGrid.startTimer(this, timeLimit, timeLeft);
                }
            }
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (proceedToNextRoom) {
                var nextRoom = selectedRoomDiv.parent().next('.room-container-block').find('.room_name_type');
...
function setRemainingPathColor(container, timeLeft) {
    ...
    if (timeLeft <= alert.threshold) {
        $(container).find(".base-timer__path-remaining").attr("class", "base-timer__path-remaining " + alert.color);
        // If the remaining time is less than or equal to 10, remove the base color and apply the "warning" class.
    } else if (timeLeft <= warning.threshold) {
        $(container).find(".base-timer__path-remaining").attr("class", "base-timer__path-remaining " + warning.color);
    }
}
...
function formatTimeLeft(time) {
    ...
    if (pasted) {
        return `-${minutes} <span class="secondly_two_dot"> : </span> ${seconds}`;
    }
    return `${minutes} <span class="secondly_two_dot"> : </span> ${seconds}`;
}

innGrid.startTimer = function (container, timeLimit, initLeft) {
    var timePassed = timeLimit - initLeft;
    var room_id = $(container).attr('data_room_id');

    $(container).find('.base-timer__label').html(formatTimeLeft(initLeft));
    setCircleDasharray(container, timeLimit, initLeft);
    setRemainingPathColor(container, initLeft);
    ...
    let timerInterval = setInterval(() => {
        ...
        $(container).find('.base-timer__label').html(formatTimeLeft(timeLeft));
        setCircleDasharray(container, timeLimit, timeLeft);
        setRemainingPathColor(container, timeLeft);
    }, 60000)
    timerIntervals['' + room_id] = timerInterval;
}
...
function stopTimer(container) {
    ...
    clearInterval(timerIntervals['' + room_id]);
}
...
function setCircleDasharray(container, timeLimit, timeLeft) {
    const circleDasharray = `${(calculateTimeFraction(timeLimit, timeLeft) * 283).toFixed(0)} 283`;
    $(container).find('.base-timer__path-remaining').attr("stroke-dasharray", circleDasharray);
}
